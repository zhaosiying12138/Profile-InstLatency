...
### Plan Version: 6 (Updated: Round 2 review)
...
| T9 Real gem5 timing suite and approval-ready quality report | AC-3, AC-6, AC-16 | needs_changes | coding | claude | Repeated real gem5 coverage is verified under `results/r01` and `results/r02` with 6442 checked-in trace files, 178/178 required groups, 3221 stable repeat groups, and marker contract PASS. The quality report remains NOT_READY because 5 blocking field-status conflicts, 29 non-identifiable ProcResource rows, and explicit human approval are unresolved. |
| T10 Raw-observation parameter search | AC-9, AC-10 | needs_changes | coding | claude | Round 2 added a shared candidate tuple path and fixed the stale real-platform search-summary equations, but the planned simulator is still incomplete: non-chainable T11 placeholders are used as latency evidence, T20 pair timing is skipped instead of eliminating ProcResource candidates, and `vcpop_m` LMUL `m4` has 5 conflicts. |
| T11 Real-platform gate enforcement | AC-16 | needs_changes | coding | claude | Gate validates trace inventory, repeat coverage, marker contract, field-status risks, and approval hashes. It correctly fails on 5 unresolved LLVM field-status conflicts plus missing explicit human approval; keep active until the real-platform gate passes. |
...
|-------|-----------------|-------------|-----------------|
| Candidate timing-model resimulation is still incomplete for non-chainable latency and T20 resource classification. | Round 2 review | AC-9 | Make the shared simulator honor template metadata: use T11 as a latency constraint only when the body is a real RAW chain, model T12 consumer gaps for non-chainable results, and use T20 pair timing to eliminate `ProcResource` candidates instead of skipping it as non-identifiable. |
| `vcpop_m` LMUL `m4` has 5 real-platform candidate conflicts. | Round 2 review | AC-9, AC-16 | Diagnose the non-linear T10/T11 curve for `vcpop_m` `m4`, add the focused repeat/diagnostic templates required to distinguish throughput, scalar destination effects, and RAW readiness, then regenerate search/profile/field-status/inventory/quality artifacts. |
| ProcResource is non-identifiable for 29 non-conflicting real-platform rows. | Round 2 review | AC-9, AC-16 | Add a T20 pair-count sweep or simulator pipe-label evidence path, update the candidate simulator to model two-pipe allocation including flexible resource groups, and regenerate field-status so `ProcResource` is inferred, conflicted, or explicitly approved rather than silently treated as complete. |
