...
### Plan Version: 5 (Updated: Round 1 review recheck)
...
| T9 Real gem5 timing suite and approval-ready quality report | AC-3, AC-6, AC-16 | needs_changes | coding | claude | Repeated real gem5 coverage is verified under `results/r01` and `results/r02` with 6442 checked-in trace files, 178/178 required groups, 3221 stable repeat groups, and marker contract PASS. The quality report remains NOT_READY because 118 field-status risks and explicit human approval are unresolved. |
| T10 Raw-observation parameter search | AC-9, AC-10 | needs_changes | coding | claude | Mode-isolated real-platform search/profile artifacts exist and use 6472 `real_platform_profile`/`gem5_minor` traces, but the planned shared candidate timing-model simulator over `Latency`, `ReleaseAtCycles`, `ProcResource`, `NumMicroOps`, and `SingleIssue` is still incomplete. Current output still has 44 conflicts and 74 insufficient-evidence rows. |
| T11 Real-platform gate enforcement | AC-16 | needs_changes | coding | claude | Gate now validates trace inventory, repeat coverage, marker contract, field-status risks, and approval hashes. It correctly fails on 118 unresolved LLVM field-status risks plus missing explicit human approval; keep active until the real-platform gate passes. |
...
| AC-16 | T11 field-status and marker-contract gate wiring | 1 | 1-review | `python3 scripts/check_calibration_gate.py --mode real_platform_profile --profile-root results` fails closed on missing approval and 118 unresolved real-platform LLVM field-status risks from `results/common/real_platform_inventory.json`. |
...
|-------|-----------------|-------------|-----------------|
| Candidate timing-model resimulation is still incomplete. | Round 1 review | AC-9 | Replace independent per-field equations with a shared candidate timing-model simulator that replays T10/T11/T12/T20/T21/T30 template bodies for `Latency`, `ReleaseAtCycles`, `ProcResource`, `NumMicroOps`, and `SingleIssue`, then selects the minimal explaining candidate set. |
| Real-platform LLVM field-status risks remain unresolved. | Round 1 review | AC-16 | Resolve the 44 `conflict` and 74 `insufficient_evidence` rows through the shared candidate simulator and follow-up experiments, or require explicit machine-readable human approval that accepts each unresolved risk by ID and artifact hash. |
...
| Real-platform search summary still documents outdated interval equations. | Round 1 review | AC-13, AC-16 | Update and regenerate the search report so its global assumptions match the implemented `(iterations - 1) * field` interval equations for T10/T11 marker pairs. |
