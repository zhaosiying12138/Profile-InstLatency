...
### Plan Version: 9 (Updated: Round 5 review)
...
| T6 Humanize2 primitive capture and replay cartridge | AC-11, AC-12, AC-13 | needs_changes | coding | claude | Round 5 removed the stale Round 1/T01-only replay claims and refreshed boards/events/replay/cartridge, but the Round 5 code worker's prompt/contract/output/tool-call artifacts are still absent from `results/common/agentic_flow/`; keep active until that capture is complete. |
...
| T11 Real-platform gate enforcement | AC-16 | needs_changes | coding | claude | Gate validates trace inventory, repeat coverage, marker contract, field-status risks, and approval hashes. It correctly fails closed on missing `Gate status: PASS` plus missing machine-readable human approval under `results/common`, but approval validation must be hardened so any approval for the 39 non-identifiable rows is tied to both current inventory and field-status hashes or explicit accepted risk IDs. |
| T12 Round 2-4 Humanize2 replay/capture refresh | AC-12, AC-13 | needs_changes | docs | claude | Round 5 refreshed the stale Round 2-5 replay/boards/events/cartridge state, but the capture is still incomplete because the Round 5 code worker's prompt, contract, output, verification, and state-changing tool-call artifacts are not registered under `results/common/agentic_flow/`. |
...
|-------|-----------------|-------------|-----------------|
| Round 5 code-worker Humanize2 capture artifacts are incomplete. | Round 5 review | AC-12, AC-13 | Add the Round 5 candidate-simulator/code worker prompt, contract, output, verification, and normalized state-changing/tool-call records under `results/common/agentic_flow/artifacts/`; register them in `h2_primitives.yaml`, update boards/events/replay/cartridge from `pending_review` to the reviewed Round 5 state, and keep the approval boundary explicit. |
| Real-platform approval validation is too weak for non-identifiable rows. | Round 5 review | AC-16 | Treat `non_identifiable` field-status rows as approval-bound risks unless stronger evidence resolves them. Require any human approval artifact to include both `inventory_sha256` and `real_platform_field_status_sha256` matching the current inventory, plus accepted risk IDs or an explicit all-risks acceptance covering the 39 non-identifiable rows; add focused checker tests for approval files that only cite trace counts or stale hashes. |
| 39 non-identifiable real-platform LLVM-facing rows require resolution or explicit human acceptance. | Round 4 review | AC-16 | Add stronger follow-up experiments/modeling for the 30 ProcResource rows, 6 T12 upper-bound-only Latency rows, and 3 `vcpop_m` LMUL `m4` issue-field rows, or obtain explicit machine-readable human approval tied to `real_platform_inventory.json` hash `d29e632b98c0a5734d541939c561872eeed691fd3c00b7ea83cf8aea666a536d` and `real_platform_field_status.json` hash `904cca46aff4a923bc230d069230e15eb164af043f020dab33e5546f18560179`. |
